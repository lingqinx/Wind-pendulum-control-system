...
void adc_init(ADCn adcn,ADC_Ch ch)
{
    ...
    case ADC0:       /*   ADC0  */
        ...
        SIM_SOPT7 &= ~(SIM_SOPT7_ADC0ALTTRGEN_MASK  |SIM_SOPT7_ADC0PRETRGSEL_MASK);// selects PDB not ALT trigger
        SIM_SOPT7 = SIM_SOPT7_ADC0TRGSEL(0);        // PDB external pin input trigger for ADC
...
*  函数返回：无符号结果值
*  修改时间：2012-2-10
*  备    注：参考苏州大学的例程，B通道不能软件触发！！！！  个人测试 在48Mbus下，AD转换一个次不到7us
*************************************************************************/
u16 ad_once(ADCn adcn,ADC_Ch ch,ADC_nbit bit)  //采集某路模拟量的AD值
{
    ...
    while (( ADC_SC1_REG(ADCx[adcn], A ) & ADC_SC1_COCO_MASK ) != ADC_SC1_COCO_MASK);
    result = ADC_R_REG(ADCx[adcn],A);
    ADC_SC1_REG(ADCx[adcn],A) &= ~ADC_SC1_COCO_MASK;
    return result;
}
...
*  备    注：修改苏州大学的例程
*************************************************************************/
u16 ad_ave(ADCn adcn,ADC_Ch ch,ADC_nbit bit,u16 N) //均值滤波
{
    u32 tmp = 0;
    u16  i;
    ASSERT( ((adcn == ADC0) && (ch>=AD8 && ch<=AD18)) || ((adcn == ADC1)&& (ch>=AD4a && ch<=AD17)) ) ;   //使用断言检测ADCn_CHn是否正常
