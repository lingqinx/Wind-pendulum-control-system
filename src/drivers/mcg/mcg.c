...
unsigned char pll_init(clk_option opt)
{
    ...
    if(opt  != PLLUSR ) //自定义模式，直接加载全局变量mcg_div的值
    {
        ...
        case PLL200:
            ...
            mcg_div.vdiv        =28;
            break;
        case PLL225: //绝大部分芯片都成超到这个程度
            mcg_div.prdiv       =11;
            mcg_div.vdiv        =30;
            break;
        case PLL250: //不同芯片，不同板子，超频能力不一样，不一定全部都能超到这个水平
            mcg_div.prdiv       =10;
            mcg_div.vdiv        =31;
            break;
        default:
...
** 	 间才可以从新使能预取功能。                                        
**        MCG=PLL   core=MCG/(outdiv1+1)               bus=MCG/(outdiv2+1)  
**                  FlexBus=MCG/(outdiv1+1)    Flash clock=MCG/(outdiv1+1)  
********************************************************************************
*/
