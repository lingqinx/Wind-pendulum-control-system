...
{
    u8  CARD_TYPE;  //保存插入设备类型
    u32 BITS;       //设置SDHC模块的读取和写入时的位宽
    u32 SD_TIMEOUT; //设置SDHC命令超时时间
    u32 NUM_BLOCKS; //插入的设备的模块数
    u8  SDHC;       //保存插入设备是否为SDHC，如果为TRUE，起始地址=扇区*512
    u8  VERSION2;   //判断是否为SD 2.0以上的卡
    u32 ADDRESS;    //设备地址
